...
#ifndef NOMINMAX
#define NOMINMAX // Định nghĩa rào chắn để tránh xung đột hàm min/max khi gọi windows.h
#endif
...
#define GREEN "\033[32m" // Đặt màu xanh lá dành riêng cho các thành công/tiêu đề
#define YELLOW "\033[33m" // Đặt màu vàng tạo điểm nhấn (phần hiển thị giá tiền)
#define CYAN "\033[36m" // Đặt màu xanh ngọc cho phần nổi bật của tiêu đề bản ghi
#define BOLD "\033[1m" // Mã đặc biệt giúp bôi đậm văn bản cần chú ý

// Hàm tiện ích để làm sạch bộ nhớ đệm, dùng để chống trôi lệnh khi xen kẽ giữa getline() và cin >>
void clearBuffer() {
  cin.clear(); // Hủy cờ lỗi trên luồng nhập khi người dùng lỡ nhập sai kiểu dữ liệu chữ vào số
...
class TaiLieu {
private:
  // Khai báo các thuộc tính chung chỉ cho quyền nội bộ theo đóng gói (Encapsulation)
  string maTaiLieu;
  string tenTaiLieu;
  string nhaXuatBan;
  int namXuatBan;
...
public:
  // Hàm Constructor (Khởi tạo) không tham số thiết lập giá trị chuẩn an toàn lúc rỗng
  ...
  // Hàm Destructor ảo đảm bảo máy tính xóa đúng bộ nhớ rác khi đối tượng dẫn xuất kết thúc
  virtual ~TaiLieu() {}

  // Tập hợp Getter / Setter theo giao diện điều khiển (interface) an toàn lấy/gán dữ liệu (2.0đ)
  ...
  // Phương thức Đa hình (ảo hóa - virtual) để xử lý bước nhập từng dữ liệu gốc riêng
  // Có thể được ghi đè bởi các lớp Sách/Báo nhờ cơ chế liên kết trễ (Late binding) - (3.0đ)
  virtual void Nhap(istream &is) {
    cout << "Nhập mã tài liệu: ";
    is >> maTaiLieu; // Nhập dữ liệu mã với từ duy nhất không có khoảng cách
    clearBuffer(); // Phải dọn bộ nhớ ngay sau đó để chuẩn bị lấy getline có đoạn trống
    ...
    is >> namXuatBan; // Yêu cầu nhập một số nguyên dùng làm năm của ấn phẩm
  }

  // Khai báo hàm ảo hiển thị chuỗi nội dung với việc đẩy cột thẳng hàng
  virtual void HienThiThongTin(ostream &os) const {
    // Ép in về phía trái của cột, với cỡ chiều rộng setw cố định cho từng loại thông tin
  ...
  // Hàm này chặn người dùng khởi tạo trực tiếp instance ảo Tài Liệu không xác định
  ...
  friend istream &operator>>(istream &is, TaiLieu &tl) {
    tl.Nhap(is); // Cầu nối lấy gọi hàm đa hình, sẽ móc nối hàm nhập chuẩn tuỳ loại object
    return is;
  }
  
  // Quá tải luồng đổ dữ liệu tạo một thao tác xuất gọn qua cout <<
  friend ostream &operator<<(ostream &os, const TaiLieu &tl) {
    tl.HienThiThongTin(os); // Cầu nối xuất lấy đúng nội dung đính kèm các đặc điểm riêng rẽ
    return os;
  }
};
...
class Sach : public TaiLieu { // Khởi xướng Kế Thừa tính năng gốc từ lớp cha lớp 'TaiLieu'
private:
...
public:
  ...
  // Nạp toàn bộ dữ liệu từ gốc đến ngọn nếu có thể xác định sớm lúc khởi tạo Sách
  ...
  void Nhap(istream &is) override {
    TaiLieu::Nhap(is); // Ra lệnh gọi phần Nhập của lớp cơ sở giải quyết 4 thông số nền đè trước
    cout << "Nhập số trang: ";
    is >> soTrang; // Thu nhận dữ liệu lượng trang
    clearBuffer(); // Đề phòng lệnh getline phía sau bị nhảy do cặn Enter
    cout << "Nhập tác giả: ";
    getline(is, tacGia); // Lấy tên tác giả (cho phép khoảng cách)
  }

  // Ghi đè phương thức Hiện thị (Tùy chỉnh khoảng cột với phép bù sizeof để UTF-8 hoạt động chuẩn)
  void HienThiThongTin(ostream &os) const override {
    TaiLieu::HienThiThongTin(os); // In 4 trường mặc định gốc trước
    // Căn trái và đẩy các trường thêm: Phân Loại (Sách), Tác Giả, Số Trang, và Tinh Tiền đi kèm định dạng màu
  ...
  double TinhTien() const override { return soTrang * 500.0; } // Đồng giá 500đ nhân với lượng trang
};
...
public:
  ...
  // Kịch bản Nhập thêm đối với loại Tạp Chí yêu cầu rà soát giá trị tháng (từ 1-12)
  void Nhap(istream &is) override {
    TaiLieu::Nhap(is); // Điền thông số chuẩn gốc cho Tạp Chí
    cout << "Nhập số phát hành: ";
    is >> soPhatHanh;
    
    // Khởi tạo khoá cản ngoại lệ kiểm soát các trị số nhập hợp lệ theo tháng (1 đến 12)
    do {
      if (is.fail()) { // Bắt lỗi gõ luồng (Vd: nhập chữ vào int) để tránh vòng lặp chết
        is.clear();
      ...
      // Nếu lọt luồng fail hoặc trị số bị lố ngoài 1 và 12, cảnh báo bằng màu RED yêu cầu thử lại
      if (is.fail() || thangPhatHanh < 1 || thangPhatHanh > 12) {
        cout << RED
             << "Lỗi: Tháng phát hành phải từ 1 đến 12. Vui lòng nhập lại!"
             << RESET << endl;
      }
    } while (is.fail() || thangPhatHanh < 1 || thangPhatHanh > 12); // Bẻ vòng khi kết quả hoàn toàn đúng đắn
  }

  // Khai báo hàm đa hình cho bộ hiển thị dữ liệu bảng điều khiển
  void HienThiThongTin(ostream &os) const override {
    TaiLieu::HienThiThongTin(os); // Ra lệnh tải cột ban đầu
    // Biến kết gộp chuỗi giúp việc xếp cột bằng thuật sizeof không bị xô dích UTF-8
...
public:
  ...
  // Xông chức năng Nhập của Báo đảm bảo thu hẹp mốc đầu vào của ngày trong thang 1..31
  void Nhap(istream &is) override {
    TaiLieu::Nhap(is); // Điền thông số chuẩn cho Báo
    
    // Tự động yêu cầu nhập đi lặp lại ngày cho tới khi thu được khung số quy chuẩn hợp lệ
    do {
    ...
    } while (is.fail() || ngayPhatHanh < 1 || ngayPhatHanh > 31); // Lệnh chốt khoá vòng nếu giá trị sạch
  }
...
// Hàm vẽ khung đỉnh và đề mục bảng hiển thị danh sách chi tiết (xử lý byte bù trừ cho UTF-8 an toàn)
void InTieuDeBang() {
  cout << string(130, '-') << endl; // Đổ ra một dải đường ngang biên giới trên độ dài 130 ký hiệu
  cout << BOLD << CYAN << left << setw(5 + sizeof("Mã TL") - 1) << "Mã TL"
       ...
       << setw(7 + sizeof("Giá Tiền") - 1) << "Giá Tiền" << RESET << endl; // Chèn mã kết thúc làm màu về mặc định
  cout << string(130, '-') << endl; // Đổ đường viền phân tách các hàng thông tin số liệu chính yếu
}

int main() {
  // Lệnh can thiệp chèn cấu trúc mã chữ hệ thống UTF-8 giúp Windows chạy được ký tự Tiếng Việt (có dấu)
  SetConsoleOutputCP(CP_UTF8);
  SetConsoleCP(CP_UTF8);
  
  // Khởi tạo một mảng động (Vector) dùng con trỏ tĩnh làm mẫu chốt (nhảy bước thiết lập mảng Đa Hình)
  vector<TaiLieu *> danhSachTaiLieu;
  int luaChon; // Biến chọn lệnh kêt nối Menu điều hướng

  // Thuật toán khối khởi động vòng Menu liên tục, lặp đến khi có hiệu lệnh Kết thúc
  do {
    ...
    cin >> luaChon; // Nhận tác động điều hướng

    TaiLieu *tlMoi = nullptr; // Chốt biến gốc con trỏ về rỗng giúp an toàn khi chọn lệch phím điều hướng

    // Khối điều rẽ hướng điền chọn bằng thẻ (1 ra Sách, 2 ra Tạp chí...)
    switch (luaChon) {
    case 1:
      cout << CYAN << "\nNhập thông tin Sách" << RESET << endl;
      tlMoi = new Sach(); // Gọi vùng cấp phát bộ nhớ mới làm thuộc tinh là một đối tượng thuộc loại Sách
      break;
    case 2:
    ...
    case 3:
      ...
      break;
    case 4:
      // Phát thông điệp cản đường người dùng trong trường mảng còn hoàn toàn trống
      if (danhSachTaiLieu.empty()) {
        cout << RED << "Danh sách trống!" << RESET << endl;
      } else {
        // Tái lập hiển thị toàn bộ nội dung nếu vector đang nắm giữ dữ liệu thành phần
        ...
        // Quét lấy tất cả thiết đặt bản lưu trong Mảng (Chạy đa hình dựa vào Type thực chất)
        for (const auto &tl : danhSachTaiLieu) {
          // Gọi thao tác đổ thẳng qua operator Toán tử (<<) đa hình để phân rã ra Sách/Báo tự động
          cout << *tl;
          tongTien += tl->TinhTien(); // Hút giá tiền của mảng để tổng hợp ngân quỹ
        }
        cout << string(130, '-') << endl; // Kẻ vạch đóng cấu tạo bảng ấn phẩm
        
        // Hiện thông báo tiền tệ kết toán với setprecision chặn không lộ dấu chấm trôi nổi (phân số thập phân)
        cout << BOLD << RED << "TỔNG TIỀN TẤT CẢ TÀI LIỆU: " << fixed
             << setprecision(0) << tongTien << " VNĐ" << RESET << endl;
      }
      break;
    case 0:
      cout << "Đang thoát chương trình...\n"; // Điểm báo dừng vòng quay console trước khi trả về System
      break;
    default:
      // Cảnh báo mã phản hồi với thao tác gõ mã chữ/số ngoài phạm trù từ 0 - 4
      cout << RED << "Lựa chọn không hợp lệ. Vui lòng nhập lại!\n" << RESET;
      clearBuffer(); // Phải phá bộ nhớ lỗi chống rác dồn vòng lặp vô tận
    }

    // Thủ diễn khép luồng cho Mảng đẩy đối tượng thêm vào khi rẽ nhánh new Obj() không thất bại (NULL)
    if (tlMoi != nullptr) {
      cin >> *tlMoi; // Tiến hành lùi về phần (>> Nhập) tự nhận hệ số đa hình theo Object Sáng lập
      danhSachTaiLieu.push_back(tlMoi); // Chuỗi hàm chèn con tem con trỏ vào đáy giỏ Vector
      cout << GREEN << "Thêm tài liệu thành công!\n" << RESET;
    }

  } while (luaChon != 0); // Neo khoá cản cho rẽ ngang ở Case 0 Exit

  // Xóa bỏ phế thải và thu hồi lại vùng heap memory mà C++ mượn làm lớp qua Vector trước khi tắt máy
  for (auto tl : danhSachTaiLieu) {
    delete tl; // Khóa gỡ hoàn toàn trỏ đang neo vào heap (khởi động gọi Destructor)
  }
  danhSachTaiLieu.clear(); // Gỡ toàn mặt xích khỏi hàng list để an toàn tuyệt đối rò rỉ RAM
